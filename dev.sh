...
if [ "$SERVER_ROLE" = "public" ]; then
    ...
    pipenv run celery -A archive worker -Q celery,public,common,maintenance --loglevel=info --concurrency=$CELERY_WORKER_CONCURRENCY -P prefork &
    pipenv run celery -A archive beat --loglevel=info &
else
    # Use spawn method on macOS for Celery
    CELERY_WORKER_CONCURRENCY=${CELERY_WORKER_CONCURRENCY:-4}
    pipenv run celery -A archive worker -Q celery,private,common,maintenance --loglevel=info --concurrency=$CELERY_WORKER_CONCURRENCY -P prefork &
    pipenv run celery -A archive beat --loglevel=info &
fi
