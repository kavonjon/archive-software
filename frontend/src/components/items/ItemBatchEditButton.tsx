...
interface ItemBatchEditButtonProps {
  mode: BatchEditMode;
...
const ItemBatchEditButton: React.FC<ItemBatchEditButtonProps> = ({
  mode,
  onModeChange,
  onExecute,
  selectedCount,
  ...
  const getTooltipText = (): string | null => {
    if (cacheLoading && cacheProgress < 100) {
      return `Loading item data: ${Math.round(cacheProgress)}%`;
    }
    return null;
  };
  ...
  const handleMenuItemClick = (selectedMode: BatchEditMode) => {
    ...
    localStorage.setItem('item-batch-edit-mode', selectedMode);
...
export default ItemBatchEditButton;
