...
const LanguoidDetail: React.FC = () => {
  ...
  // Temporary marker for map click-to-set coordinates
  ...
  const saveBothCoordinates = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      const updatedLanguoid = await languoidsAPI.patch(languoid.id, {
        latitude,
        longitude
      });
      
      // Update the languoid state with the new data
      setLanguoid(updatedLanguoid);
  ...
  // Map components for Geographic Information card
  ...
  // Handler for map click to set coordinates
  ...
  // Handler for setting coordinates from temp marker
  const handleSetCoordinates = async () => {
    if (tempMarker && languoid) {
      try {
      ...
      <Card sx={{ elevation: 1 }}>
        <CardContent>
          ...
          {(() => {
            ...
            const lat = languoid.latitude !== null ? Number(languoid.latitude) : null;
            const lon = languoid.longitude !== null ? Number(languoid.longitude) : null;
            ...
            const center: [number, number] = hasCoordinates ? [lat!, lon!] : defaultCenter;
            const zoom = hasCoordinates ? 4 : defaultZoom;
            
            return (
              <Box sx={{ height: 300, mb: 3, borderRadius: 1, overflow: 'hidden' }}>
                <MapContainer
                  center={center}
                  zoom={zoom}
                ...
                >
                  ...
                  {/* Existing languoid marker */}
                  {hasCoordinates && (
                    <Marker position={[lat!, lon!]}>
                      <Popup>
                        <strong>Languoid Location</strong>
                        <br />
                        Lat: {lat!.toFixed(6)}
                        <br />
                        Lon: {lon!.toFixed(6)}
